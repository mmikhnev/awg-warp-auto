...
return view.extend({
	...
	setWarpAutoSettings: function(settings) {
		...
		fields.provider.value = settings.provider || 'native';
	...
	renderWarpAutoPool: function(state) {
		...
		var configuredProvider = (state.settings && state.settings.provider) || 'native';
	...
	runWarpAutoAction: function(action, profileId) {
		...
		if (action == 'clean_replenish') {
			...
			return new Promise(L.bind(function(resolve) {
				ui.showModal(_('Clean Inactive Profiles'), [
					E('p', [
						_('Are you sure you want to delete %d inactive profile(s)? The active profile will be preserved.').format(inactiveCount)
					]),
					E('div', { 'class': 'cbi-value', 'style': 'margin-top: 15px;' }, [
						E('label', { 'class': 'cbi-value-title', 'style': 'width: 60%;' }, [
							_('Generate new profiles after cleanup:')
						]),
						E('div', { 'class': 'cbi-value-field', 'style': 'display: flex; align-items: center; gap: 8px;' }, [
							replenishInput,
							E('span', { 'class': 'cbi-value-description' }, [
								_('(0 = only delete, 1-10 = generate candidates)')
							])
						])
					]),
					E('div', { 'class': 'right', 'style': 'margin-top: 20px;' }, [
						...
						E('button', {
							...
							'click': L.bind(function(ev) {
								...
								var count = parseInt(replenishInput.value, 10);
								if (isNaN(count) || count < 0) count = 0;
								if (count > 10) count = 10;
								ui.hideModal();
								resolve(this.runCleanReplenishFlow(count));
							}, this)
						}, [ _('Delete & Replenish') ])
					])
				]);
	...
	runCleanReplenishFlow: function(replenishCount) {
		var count = (replenishCount != null && replenishCount !== '') ? parseInt(replenishCount, 10) : 0;
		if (isNaN(count) || count < 0) count = 0;
		if (count > 10) count = 10;

		this.setWarpAutoBusy(true);
		this.showWarpAutoMessage(count > 0
			? _('Deleting inactive profiles and generating %d new profile(s)…').format(count)
			: _('Deleting inactive profiles…'));

		return callSaveWarpAutoSettings(this.getWarpAutoSettings(), '1').then(L.bind(function(saved) {
			if (!saved || saved.ok === false)
				throw new Error(saved && saved.error || _('Unable to save WARP Auto settings'));
			this.autoSettingsDirty = false;
			return callWarpAutoAction('clean_replenish', String(count));
		}, this)).then(L.bind(function(result) {
			if (!result || result.ok === false)
			...
			if (count > 0) {
				...
				this.showNotification(_('Inactive profiles deleted successfully.'), 'info');
				this.showWarpAutoMessage(_('Inactive profiles deleted.'));
				return this.updateWarpAuto();
			}
		}, this)).catch(L.bind(function(error) {
			try { sessionStorage.removeItem('warp_auto_active_batch'); } catch(e) {}
	...
	renderWarpAuto: function() {
		...
		var cleanButton = E('button', {
			'class': 'btn cbi-button cbi-button-negative',
			'title': _('Delete all profiles except currently active and optionally generate new candidates'),
			'click': L.bind(function(event) {
				event.preventDefault();
