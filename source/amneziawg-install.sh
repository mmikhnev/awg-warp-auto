...
download_package() {
    ...
    if download_url "$preferred_url" "$awg_dir/$preferred_file" && [ -s "$awg_dir/$preferred_file" ]; then
        echo "$preferred_file"
        return 0
    fi
    ...
    if download_url "$fallback_url" "$awg_dir/$fallback_file" && [ -s "$awg_dir/$fallback_file" ]; then
        echo "$fallback_file"
        return 0
    fi
