...
return view.extend({
	...
	setWarpAutoBusy: function(busy) {
		...
		(this.autoActionButtons || []).forEach(function(button) {
		...
		if (!busy && this.autoActivateButton)
			this.autoActivateButton.disabled = !(this.autoCandidate && this.autoCandidate.value);
	...
	renderWarpAutoRuntime: function(state) {
		...
		dom.content(this.autoRuntimeNode, ui.itemlist(E([]), [
			...
			_('Active profile'), String(active || '-'),
	...
	renderWarpAutoPool: function(state) {
		...
		var previous = this.autoCandidate.value;
		var options = [ E('option', { 'value': '' }, [ _('Select a READY profile') ]) ];
		var rows = pool.map(function(entry) {
			...
			if (status == 'READY')
				options.push(E('option', { 'value': id }, [ String(profile) + ' (' + String(endpoint) + ')' ]));
		...
		if (!rows.length)
			rows.push(E('tr', [ E('td', { 'colspan': 6 }, [ E('em', [ _('No WARP Auto profiles yet.') ]) ]) ]));
		...
		dom.content(this.autoCandidate, options);
		if (previous && options.some(function(option) { return option.value == previous; }))
			this.autoCandidate.value = previous;
		this.autoActivateButton.disabled = this.autoBusy || !this.autoCandidate.value;
	},
	...
	runWarpAutoAction: function(action) {
		...
		var id = action == 'activate' ? this.autoCandidate.value : '';
		var actionNames = {
	...
	renderWarpAuto: function() {
		...
		var healthInterval = makeInput('health_interval', 'number', { 'min': 10, 'step': 1 });
		...
		var candidate = E('select', { 'class': 'cbi-input-select' }, [
			E('option', { 'value': '' }, [ _('Select a READY profile') ])
		]);
		...
		var activateButton = E('button', {
			'class': 'btn cbi-button cbi-button-positive',
			'disabled': true,
			'click': L.bind(function(event) {
				event.preventDefault();
				return this.runWarpAutoAction('activate');
			}, this)
		}, [ _('Activate selected') ]);
		...
		candidate.addEventListener('change', L.bind(function() {
			this.autoActivateButton.disabled = this.autoBusy || !candidate.value;
		}, this));

		this.autoFields = fields;
		this.autoFields.critical_resource = critical;
		this.autoFields.log_level = logLevel;
		this.autoCandidate = candidate;
		this.autoSaveButton = saveButton;
		this.autoActivateButton = activateButton;
		this.autoBootstrapButton = bootstrapButton;
		this.autoActionButtons = [ bootstrapButton, refreshButton, testButton, activateButton, rollbackButton ];
		...
		return this.renderWarpTabs([
			{
				...
				title: _('Overview'),
				content: [
					E('p', [ _('Quick start, runtime state and tested WARP profile pool.') ]),
					E('h3', [ _('Quick start') ]),
					E('div', { 'class': 'cbi-page-actions' }, [
						bootstrapButton,
						' ',
						E('span', [ _('Creates selected missing interface. Existing interface is never replaced by this action.') ])
					]),
					...
					E('div', { 'class': 'cbi-value' }, [
						E('label', { 'class': 'cbi-value-title' }, [ _('Ready profile') ]),
						E('div', { 'class': 'cbi-value-field' }, [ candidate, ' ', activateButton ])
					]),
					E('div', { 'class': 'cbi-page-actions' }, [ refreshButton, ' ', testButton, ' ', rollbackButton, this.autoMessageNode ])
				]
			},
			{
				id: 'settings',
				...
				content: [
					...
					makeRow(_('Health-check interval'), healthInterval, _('seconds; default 60')),
			...
			},
			{
				...
				content: [ this.manualImporterNode ]
			},
			{
				id: 'status',
				title: _('AmneziaWG status'),
				content: [ this.statusNode ]
			},
			{
				id: 'logs',
				title: _('Logs'),
			...
			return callValidateAwgConfig(configText, file.name).then(L.bind(function(result) {
				...
				ui.showModal(_('Apply AmneziaWG profile'), [
					...
					ui.itemlist(E([]), [
						...
						_('Preserved settings'), 'ListenPort 51821, fwmark 0x08000000, route_allowed_ips 0',
