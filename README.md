...
- **Быстрый импорт любых сторонних конфигов AmneziaWG / WireGuard:**
  - Во вкладке **Services -> AmneziaWG** встроен удобный импортер конфигураций.
  - Можно загрузить `.conf` файл любого VPN (WireGuard, AmneziaWG 1.0, 2.0, 3.0, 3.1 или со своего личного VPS).
  - Парсер автоматически распознает и пропишет все параметры обфускации (`Jc`, `Jmin`, `Jmax`, `S1`–`S4`, `H1`–`H4`, `I1`–`I5`, `ContentPaddingAddition`, таймеры `Rekey` и т.д.) в выбранный интерфейс роутера без ручной возни с UCI.
  - Сразу после применения можно выполнить автоматическую проверку доступности сети через туннель.
